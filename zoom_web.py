...
def set_mem1():
    if os.path.isfile("data/memory/mem1.txt"):
        os.remove("data/memory/mem1.txt")
    with open("data/memory/mem1.txt", "x") as f1:
...
def read_mem1():
    f1r = open("data/memory/mem1.txt","r")
    mem1_data = f1r.read()
    webbrowser.open("https://pwa.zoom.us/wc/join/"+mem1_data, 1)
...
saved_b = Button(root, text="Open Mem 1", bg="deepskyblue2", fg="white", font=myfont, command= read_mem1)
saved_b.place(x=int(width/4*1), y=row4, anchor="center")
set_b = Button(root, text="Save Mem 1", bg="deepskyblue2", fg="white", font=myfont, command=set_mem1)
